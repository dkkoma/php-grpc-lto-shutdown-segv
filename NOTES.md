...
the script runs `php grpc_call.php` in a loop with `ulimit -c
unlimited`. When the kernel writes a `core` file, gdb post-mortem
dumps `info threads` / `bt` / `info sharedlibrary` / `x/10i $pc` on
it. Running the inferior under gdb directly suppresses the race
(heisenbug — observed 0/300 under gdb-as-parent vs 6/20 with native
core dumps), so the harness uses cores instead.

Trimmed real capture from `php85-lto-forensics`, iteration 6 of 20:

```
Core was generated by `/usr/local/bin/php ...'.
Program terminated with signal SIGSEGV, Segmentation fault.
#0  0x0000ffffb259a434 in ?? ()
[Current thread is 1 (Thread 0xffffb0ebdf60 (LWP 132))]

(gdb) info threads
* 1    LWP 132   0x0000ffffb259a434 in ?? ()
  2    LWP 129   0x0000ffffb3ef507c in ?? () from libc.so.6     (php main)
  3    LWP 133   0x0000ffffb3f4bb24 in syscall () from libc.so.6
  4-15           syscall () from libc.so.6 / 0x0000ffffb2784864 in ??

(gdb) thread apply 1 bt
#0  0x0000ffffb259a434 in ?? ()
Backtrace stopped: previous frame identical to this frame (corrupt stack?)

(gdb) thread apply 2 bt          # php main
#0  0x0000ffffb3ef507c in ?? () from libc.so.6
#1  free () from libc.so.6
#2  zend_hash_destroy ()
#3  zend_interned_strings_dtor ()
#4  php_module_shutdown ()
#5  ...
#6  __libc_start_main () from libc.so.6
#7  _start ()

(gdb) thread apply 5 bt          # representative grpc worker
#0  syscall () from libc.so.6
#1  0x0000ffffb2784864 in ?? ()
Backtrace stopped: previous frame identical to this frame (corrupt stack?)
...
(gdb) info registers
pc  0x0000ffffb259a434

(gdb) x/10i $pc
=> 0xffffb259a434: Cannot access memory at address 0xffffb259a434
```

Main is mid-PHP-shutdown (`php_module_shutdown` → ... → `dlclose`),
the 13 grpc worker threads each have their immediate caller frame at
`0xffffb2784864 in ??` (an address that *was* grpc.so text), the
crashing thread's `$pc` falls in the same vacated range, grpc.so is
absent from `info sharedlibrary`, and the kernel cannot fetch an
instruction from `$pc`. Same shape on every captured core; only the
specific address inside the unmapped range varies.

With `RTLD_NODELETE`, grpc.so's text stays mapped past `dl_fini`, so
